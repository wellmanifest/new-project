...
export async function runPythonSemanticVerifier(
  ...
  try {
    ...
    const stdout = await runPython(
      ...
      cwd,
      withVerifierPythonPath(options.env ?? process.env)
    );
...
function withVerifierPythonPath(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const separator = process.platform === "win32" ? ";" : ":";
  return {
    ...env,
    PYTHONPATH: env.PYTHONPATH ? `verifier${separator}${env.PYTHONPATH}` : "verifier"
  };
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      ...
      if (code === 0) resolve(Buffer.concat(stdout).toString("utf8"));
        ...
        reject(new Error(errorOutput || `Python semantic verifier failed with exit code ${code}`));
    });
  });
}
