...
import { compareSubset, compactStableJson } from "@office-dsl/regression-runner";
import { extractPdfText } from "@office-dsl/pdf-generator";
import {
  runDocumentProcesses,
  runMockOcr,
  DOCUMENT_PROCESS_TEST_VERSION,
  type DocumentProcess,
  type DocumentProcessResult,
  type DocumentProcessTest
} from "@office-dsl/document-ingestion";
export { ocrPdfToMarkdownFixture } from "@office-dsl/document-ingestion";
import {
...
import {
  ...
  type ChatOutcome,
  type ChatRunResult
} from "@office-dsl/chat-negotiation";

export const RECRUITMENT_SCENARIO_VERSION = "example-recruitment.scenario.v1";
export const RECRUITMENT_DOCUMENT_PROCESS_TEST_VERSION = DOCUMENT_PROCESS_TEST_VERSION;
export type RecruitmentDocumentProcess = DocumentProcess;
...
export type RecruitmentDocumentProcessTest = DocumentProcessTest;
export type RecruitmentDocumentProcessResult = DocumentProcessResult;
...
function createRecruitmentProposal(
  ...
  const hash = sha256(compactStableJson(proposalWithoutHash));
...
async function discoverCandidateDirs(scenarioDir: string): Promise<string[]> {
  ...
  return entries
    .filter((entry) => {
      const candidateDir = path.join(scenarioDir, entry.name);
      return (
        entry.isDirectory() &&
        existsSync(path.join(candidateDir, "in")) &&
        existsSync(path.join(candidateDir, "chat.txt"))
      );
    })
