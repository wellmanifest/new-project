...
- **Target Repository Execution**: When assigned a task to create System X, the agent:
  ...
  11. **COORDINATE PARALLEL WORK**: Serialize ticket-ID allocation before branching, then use separate branches/worktrees and explicit `dependsOn`/`conflictsWith`. Shared contracts are edited only by the manifest-declared integration workstream; `integrationTicket` records coordination but does not transfer path ownership. One agent implements; a second agent may review read-only or own a non-overlapping ticket.
   ...
   16. **CLEAN UP LOCAL WORKSPACES**: At merge, publication or explicit pilot discard, inventory every temporary linked worktree and duplicate clone. Verify dirty state and whether HEAD is integrated or explicitly disposable before removal. Preserve unknown or unique data. Remove an exact linked worktree through `git worktree remove`, then prune metadata and delete only its released disposable local branch; prefer recoverable trash for a verified duplicate clone. Run the adopted `.governance/workspace_lifecycle_check.py` through Goal for the terminal audit. CI validates GitHub state separately and cannot see a developer filesystem.
