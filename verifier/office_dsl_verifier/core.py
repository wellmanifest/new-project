...
def verify(nl_command: str, dsl: dict[str, Any], plan: dict[str, Any], trace: dict[str, Any] | None = None, mode: str | None = None) -> VerificationReport:
...
def _mock_verify(nl_command: str, dsl: dict[str, Any], plan: dict[str, Any], trace: dict[str, Any] | None) -> VerificationReport:
    text = json.dumps({"nl": nl_command, "dsl": dsl, "plan": plan, "trace": trace}, ensure_ascii=False).lower()
    unauthorized: list[str] = []
    violations: list[str] = []
    requires_confirmation: list[str] = []
    if "nie wysy" in nl_command.lower() and "email.send" in text:
        unauthorized.append("email.send")
    if "email.send" in text:
    ...
    return VerificationReport(
        ...
        explanation="Mock verifier checks unauthorized actions, confirmations, command attempts and secret reads.",
        recommended_action=action,
    )


def _openrouter_verify(nl_command: str, dsl: dict[str, Any], plan: dict[str, Any], trace: dict[str, Any] | None) -> VerificationReport:
    ...
    response = litellm.completion(
        ...
        messages=[
            ...
            {"role": "user", "content": json.dumps({"nl_command": nl_command, "dsl": dsl, "plan": plan, "trace": trace}, ensure_ascii=False)},
        ],
