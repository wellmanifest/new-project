...
Skrypty automatyzujące (`new-ticket.sh`, `readme.sh`) odczytują niniejsze szablony i podstawiają zmienne (`{NAME}`, `{PROVIDER}`, `{NNN}`, `{TIMESTAMP}`) podczas generowania plików w katalogu `project/`.
